...
class WebService {
  ...
  static Future<List<Record>> getListRecordsOnDay(DateTime date) async {
    ...
    try {
      Response response = await Dio().get(
          "https://anika-cs.by/apptrueandfalse/log_player/record/${dateFormated.format(date)}");
  ...
  static Future<List<Record>> getListRecords() async {
    Response response = await Dio()
        .get("https://anika-cs.by/apptrueandfalse/log_player/player");
    final parsed = jsonDecode(response.data);
  ...
  static Future<bool> addRecordNowDate(Record record) async {
    ...
    Response response = await Dio().post(
        "https://anika-cs.by/apptrueandfalse/log_player/record/${record.name}/${record.nickName}/${Record.getStatus(record.status)}/${dateFormat.format(DateTime.now())}");
    final parsed = jsonDecode(response.data);
  ...
  static Future<List<DateTime>> getDateRecord(Record record) async {
    Response response = await Dio().get(
        "https://anika-cs.by/apptrueandfalse/log_player/player/${record.name}/${record.nickName}");
    final parsed = jsonDecode(response.data);
